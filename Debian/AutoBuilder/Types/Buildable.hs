...
-- DebianBuildTree.
prepareTarget :: (MonadIOError e m, HasLoc e, MonadMask m, HasRsyncError e, MonadOS r s m, T.Download a) => C.CacheRec -> Buildable a -> m (Target a)
...
prepareBuild :: (MonadIOError e m, HasLoc e, MonadMask m, HasRsyncError e, MonadOS r s m, T.Download a) => C.CacheRec -> a -> m DebianBuildTree
prepareBuild _cache target =
    ...
    either (\ (_ :: SomeException) ->
           ...
           copySource
    where
      copySource :: (MonadIOError e m, HasLoc e, MonadOS r s m) => DebianSourceTree -> m DebianBuildTree
      ...
      copyBuild :: (MonadIOError e m, HasLoc e, MonadMask m, HasRsyncError e, MonadOS r s m) => DebianBuildTree -> m DebianBuildTree
      copyBuild debBuild =
          do root <- view (osRoot . to _root . rootPath) <$> getOS
